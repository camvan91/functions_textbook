...
\begin{defn}[Exponential Function]\label{expfcndefn}    An \index{function ! exponential}\index{exponential function ! definition of}\textbf{exponential function} is the function of the form
...
\begin{thm}[Properties of Exponential Functions] \label{expfcnprops} Suppose $f(x) = b^{x}$. \index{exponential function ! graphical properties of}
...
Exponential functions also inherit the basic properties of exponents\co{ from Theorem \ref{exponentprops}}.  We formalize these below and use them as needed in the coming examples.

\smallskip

\colorbox{ResultColor}{\bbm

\begin{thm}[Properties of Exponential Functions] \label{algpropexpfcns}
...
The number $e$ is called the \index{natural base}\index{exponential function ! natural base}`\textbf{natural base}' for lots of reasons, one of which is that it `naturally' arises in the study of growth functions in Calculus.  \co{We will more formally discuss the origins of $e$  in Section \ref{ExpLogApplications}.
...
Some remarks about Example \ref{cardepreciationex} are in order.  First the function in the previous example is  called a `decay curve'.  Increasing exponential functions are used to model `growth curves'\co{ and we shall see several different examples of those in Section \ref{ExpLogApplications}}.  
...
Hence $f(x+1) = f(x) \cdot b$.  \co{This will prove useful to us in Section \ref{ExpLogApplications} when making decisions about whether or not a data set represents exponential growth or decay. } 
...
\begin{defn}[Point-base Form]\label{expfcnpointbaseform}  The \index{exponential function ! point-base form} \index{point-base form ! of an exponential function} \textbf{point-base form} of the exponential function $f(x) = a \cdot b^{x}$  is
...
\begin{defn}[Relative Rate of Change] \label{rrc}  Let $f$ be a function defined on the interval $[a,b]$ where $f(a) \neq 0$.
...
\item  We can determine the behavior of $T(t)$ as $t \rightarrow \infty$ two ways.  \co{First, we can employ  the `number sense' developed in Chapter \ref{RationalFunctions}.}

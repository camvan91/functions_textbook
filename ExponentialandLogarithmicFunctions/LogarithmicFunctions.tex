...
\begin{defn}[Base \boldmath $b$ Logarithm Function] \label{logfcndefn} For the exponential function $f(x) = b^{x}$,  $f^{-1}(x) = \log_{b}(x)$ is called the \index{function ! logarithmic} \textbf{base \boldmath $b$ logarithm function}.  We read `$\log_{b}(x)$' as `log base $b$ of $x$.' \index{logarithm ! general, ``base $b$''}
...
\begin{thm}[Properties of Logarithmic Functions] \label{logfcnprops} 
Suppose $f(x) = \log_{b}(x)$. \index{logarithm ! graphical properties of}
...
\input{ExercisesforLogarithmicFunctions}

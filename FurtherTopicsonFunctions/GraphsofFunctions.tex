...
\co{Last, and according to some\footnote{Jeff} least, w}We briefly review the notion of symmetry in the graphs of functions. \co{Recall from Definition \ref{evenfunctiondefn} that a}A function $f$ is called \textbf{even} if $f(-x) = f(x)$ for all $x$ in the domain of $f$.  The graphs of even functions are symmetric about the vertical (usually $y$-) axis.  In a similar manner, \co{Definition \ref{oddfunctiondefn} tells us a}A function $f$ is \textbf{odd} if $f(-x) = -f(x)$ for all $x$ in the domain of $f$.  Geometrically, the graphs of odd functions are symmetric about the origin.  
...
Our last example focuses on symmetry.  \co{The reader is encouraged to review the notes about symmetry as summarized on  page \pageref{reflectionsinabox} in Section \ref{AppCartesianPlane}}.
